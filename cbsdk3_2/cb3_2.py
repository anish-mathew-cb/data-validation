...
def set_bucket(type,bucket):
    cb4_1.CBConnector32.set_dest(type.upper()) 
    cb4_1.CBConnector32.set_bucket(bucket)
    return 'set_bucket'
...
def set_collection(type, bucket, scope, collection):
    ...
    cb4_1.CBConnector32.set_dest(type.upper()) 
    cb4_1.CBConnector32.set_bucket(bucket)
    cb4_1.CBConnector32.set_collection(scope, collection)
...
def get_doc(type, id):
    doc = {}
    cb4_1.CBConnector32.set_dest(type.upper()) 
    doc = cb4_1.CBConnector32.get_doc(id)
...
def init():
    try:
        ...
        cb4_1.CBConnector32.set_dest("SRC" if request.json["type"] is None else request.json["type"].upper() ) # type: ignore
        cb_connector = cb4_1.CBConnector32.init(nodes, username, password, secured )
